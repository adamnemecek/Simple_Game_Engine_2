...
namespace Entities
{
   ...
   void Controller_Component::update()
   {
      ...
      if (active_actions & ACTION_LIST::YAW_LEFT)
      {
         ...
         new_orientation = Math::F_Quat::generate_rotator_for_dual_quat(Utilities::Default_Vectors::WORLD_UP_VECTOR, -ROTATION_SPEED) * new_orientation;
      }
      ...
      if (active_actions & ACTION_LIST::PITCH_FORWARD)
      {
         ...
         new_orientation = Math::F_Quat::generate_rotator_for_dual_quat(Utilities::Default_Vectors::WORLD_LEFT_VECTOR, +ROTATION_SPEED) * new_orientation;
      }
      ...
      if (active_actions & ACTION_LIST::ROLL_LEFT)
      {
         ...
         new_orientation = Math::F_Quat::generate_rotator_for_dual_quat(Utilities::Default_Vectors::WORLD_FORWARD_VECTOR, +ROTATION_SPEED) * new_orientation;
      }
      ...
      Math::F_Quat quat_pos = Math::F_Quat::generate_pure_quat(new_position);
      quat_pos = (1 / 2.0f) * quat_pos * new_orientation;

      Math::F_Dual_Quat new_state(new_orientation, quat_pos);
