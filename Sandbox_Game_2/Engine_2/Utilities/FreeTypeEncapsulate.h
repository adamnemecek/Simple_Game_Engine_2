...
namespace Utilities
{
    class __declspec(dllexport) FreeTypeEncapsulate
    {
    public:
        static FreeTypeEncapsulate &GetInstance();
        ~FreeTypeEncapsulate();

        // takes: file path relative to solution directory, a shader program that has already been constructed with the FreeType shaders
        bool Init(const std::string &trueTypeFontFilePath, const int programId);
    ...
    private:
        ...
        FreeTypeEncapsulate();
        FreeTypeEncapsulate(const FreeTypeEncapsulate&);
        FreeTypeEncapsulate &operator=(const FreeTypeEncapsulate &);
