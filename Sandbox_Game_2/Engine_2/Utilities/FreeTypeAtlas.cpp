...
namespace Utilities
{
    ...
    bool FreeTypeAtlas::Init(const FT_Face face, const int fontPixelHeightSize)
    {
        ...
        // Note: MUST bind BEFORE setting vertex attribute array pointers or it WILL crash.  The 
        // GPU operates on the values set in the vertex attribute pointers and not on the buffer 
        // ID.  The buffer ID is simply a way to tell OpenGL to activate a certain part of the 
        // context, and then the following vertex attribute data tells OpenGL how the next draw 
        // call is going to go down.  This is a GL context thing and those will only work on the 
        // currently-set buffer.  No error is thrown if the current buffer's ID is 0 (no buffer), 
        // and that just begs for a silent bug.  A lot of OpenGL code does not clean up the 
        ...
        // Note: Remember that each character is drawn with a texture box (quad), so the 
        // required number of vertices is 4x the number of characters.
        glBufferData(GL_ARRAY_BUFFER, _MAX_STR_LENGTH * 4 * sizeof(point), 0, GL_DYNAMIC_DRAW);

        // now set up the vertex attributes, which are a property of the data organization and 
        // not the size or contents and can therefore be set after the buffer size
        ...
        const float userScale[2], const float color[4]) const
    {
        ...
        // MUST bind a buffer before sending the vertex info
        glBindBuffer(GL_ARRAY_BUFFER, _vboId);
        ...
        const float userScale[2], const float color[4]) const
    {
        ...
        // Ex: If the string were "ABC", then "B" draws further right than "A", and "C" further 
        // right than "B".
        float glyphOriginX = posScreenCoord[0];
        float glyphOriginY = posScreenCoord[1];

        // set aside memory so that screen coordinate and texture coorninate info can be drawn with 
        // a single draw call
        // Note: Each character has 4 points.  See comments on "box" in RenderChar(...) for more 
        // detailed comments.
        std::vector<point> glyphBoxes(4 * str.length());

        // I don't want to make this whole function non-const just to resize max string length, 
        // so instead force compliance to this max value
        int maxIndex = str.length();
        if (maxIndex > _MAX_STR_LENGTH)
        {
            maxIndex = _MAX_STR_LENGTH;
        }
        ...
        for (int charIndex = 0; charIndex < maxIndex; charIndex++)
        {
            ...
            // could these be condensed into the "scaled glyph" calulations? yes, but this is 
            // clearer to me
            ...
            // unlike my project "freeglut_glload_render_freetype", which loads glyphs into their
            // own textures one at a time (crude, but conveys basics), I can no longer use the 
            // whole texture and must use the offset info that was stored when the atlas was 
            // created
            // Note: Remember that textures use their own 2D coordinate system (S,T) to avoid 
            // confusion with screen coordinates (X,Y).
            float sLeft = _glyphCharInfo[c].tx;//0.0f;
            float sRight = _glyphCharInfo[c].tx + _glyphCharInfo[c].nbw;//1.0f;
            float tBottom = _glyphCharInfo[c].ty;
            float tTop = _glyphCharInfo[c].ty + _glyphCharInfo[c].nbh;

            // OpenGL draws triangles, but a rectangle needs to be drawn, so specify the four 
            // corners of the box in such a way that GL_LINE_STRIP will draw the two triangle 
            // halves of the box
            point box[4] = {
                { screenCoordLeft, screenCoordBottom, sLeft, tTop },
                { screenCoordRight, screenCoordBottom, sRight, tTop },
